...
export function ChallengeCard({ challenge, variant = 'default', onPress }: ChallengeCardProps) {
    ...
    return (
      <TouchableOpacity onPress={handlePress} activeOpacity={0.7}>
        ...
        >
          ...
          <YStack flex={1}>
            <Text fontSize={15} fontWeight="600" color="#1f2937" numberOfLines={1}>
              {challenge.name}
            </Text>
  ...
  return (
    <TouchableOpacity onPress={handlePress} activeOpacity={0.7}>
      <YStack backgroundColor="white" borderRadius="$4" padding="$4" gap="$3">
        {/* Header */}
        <XStack justifyContent="space-between" alignItems="flex-start">
          <YStack flex={1} gap="$1">
            <Text fontSize={17} fontWeight="700" color="#1f2937">
              {challenge.name}
            </Text>
        ...
        {challenge.description && (
          <Text fontSize={14} color="#4b5563" numberOfLines={2}>
            {challenge.description}
          </Text>
        )}
